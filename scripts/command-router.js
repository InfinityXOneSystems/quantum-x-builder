...
const COMMAND_PATTERNS = {
  ...
  createPR:
    /(?:create|open) (?:a )?(?:new )?(?:pull request|pr) from ([^\s]+) to ([^\s]+)(?:(?: with)? (?:title|name)[:\s]+(.+))?/i,
...
async function handleGenericCommand(command) {
  ...
  return {
    ...
    suggestion:
      'Try commands like: "create a branch called feature/new", "open a PR from develop to main", "create an issue for bug tracking"',
  };
}
